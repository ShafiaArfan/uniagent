...
# Step 1: Inject Streamlit Secret into os.environ immediately
if "GEMINI_API_KEY" in st.secrets:
    os.environ["GEMINI_API_KEY"] = st.secrets["GEMINI_API_KEY"]

from backend.config import Config

# Step 2: Safe manual validation fallback instead of crashing on Config.validate()
if not os.getenv("GEMINI_API_KEY"):
    st.error("🚨 GEMINI_API_KEY is missing! Please add it in Streamlit Cloud Settings > Secrets.")
    st.stop()
...
# Load stored documents into memory safely
load_documents()
